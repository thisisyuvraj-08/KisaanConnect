...
window.onload = function() {
	// Attach delete account button reliably and cleanup all user data
	const deleteAccountBtn = document.getElementById('deleteAccountBtn');
	if (deleteAccountBtn) {
		deleteAccountBtn.onclick = async function() {
			if (!confirm('Are you sure you want to delete your account? This action cannot be undone.')) return;
			// Unsubscribe listeners to avoid UI bugs
			if (typeof myProduceUnsub === 'function') myProduceUnsub();
			if (typeof myRequestsUnsub === 'function') myRequestsUnsub();
			if (typeof farmerPostsUnsub === 'function') farmerPostsUnsub();
			if (typeof storeRequestsUnsub === 'function') storeRequestsUnsub();
			if (currentUser && currentUser.phone) {
				try {
					// Delete all user's produce
					const produceSnap = await db.collection('farmerPosts').where('user', '==', currentUser.phone).get();
					await Promise.all(produceSnap.docs.map(doc => doc.ref.delete()));
					// Delete all user's requests
					const requestSnap = await db.collection('storeRequests').where('user', '==', currentUser.phone).get();
					await Promise.all(requestSnap.docs.map(doc => doc.ref.delete()));
					// Delete user document
					await db.collection('users').doc(currentUser.phone).delete();
					localStorage.clear();
					alert('Account and all your posts/requests deleted. You must register again to use the website.');
...
produceForm && produceForm.addEventListener('submit', function(e) {
	...
	reader.onload = async function(evt) {
		...
		await db.collection('farmerPosts').add(post);
		listenFarmerPosts();
...
function initMap() {
	...
	// Show other relevant locations
	if (userRole === 'farmer') {
		// Show kirana store requests on map
		db.collection('storeRequests').get().then(snapshot => {
			snapshot.forEach(doc => {
				const req = doc.data();
				// Only show within distance
				const myDist = Number(document.getElementById('produceDistance')?.value || 10);
		...
		// Show farmer produce posts on map
